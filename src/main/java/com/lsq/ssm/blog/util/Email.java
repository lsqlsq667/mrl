package com.cwn.wethink.support.email;
...
public class Email {
    ...
    private String auth = "false";    //是否启用验证
    ...
    private void init() {
        ...
        props.put("mail.smtp.auth", auth); // 需要身份验证
        ...
        session = Session.getDefaultInstance(props, new EmailAuthenticator(sendUserName, sendUserPass));

        //-------
        //Session mailSession = Session.getInstance(props,new MyAuthenticator(sendUserName,sendUserPass));
    ...
    private Email(String smtpHost, String sendUserName, String sendUserPass,
     ...
     * @return
     */
    public static Email entity(String smtpHost, String sendUserName, String sendUserPass,
                               String to, String cc, String mailSubject, String mailBody,
                               List<String> attachments) {
        return new Email(smtpHost, sendUserName, sendUserPass, to, cc, mailSubject,
    ...
    private boolean setBody(String mailBody) {
        try {
            ...
            log.error("设置邮件正文时发生错误 setBody 失败：", e);
            return false;
        }
        return true;
    }
    ...
    public boolean addFileAffix(String filename) {
        try {
            if (filename != null && filename.length() > 0) {
                ...
                bp.setFileName(MimeUtility.encodeText(dataSource.getName(), "utf-8", null)); // 解决附件名称乱码
                mp.addBodyPart(bp);// 添加附件
                files.add(dataSource);
            }
        } catch (Exception e) {
            log.error("增加邮件附件 addFileAffix 失败：", e);
            return false;
        }
        return true;
    }
        ...
        try {
            ...
            for (Iterator<FileDataSource> it = files.iterator(); it.hasNext(); ) {
                ...
                if (dataSource != null && dataSource.getFile() != null) {
                    dataSource.getFile().delete();
                }
            }
        } catch (Exception e) {
            log.error("delFileAffix 失败：", e);
            return false;
        }
        return true;
    }
